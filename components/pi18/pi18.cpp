  ...
  // 2) GS (general status)
  {
    ...
    if (read_line_(line, 800)) {
      ...
      if (dpos != std::string::npos && line.size() > dpos + 5) {
        ...
        if (cut != std::string::npos) csv.resize(cut);
...
bool PI18Component::parse_gs_(const std::string &payload) {
  ...
  if (start < payload.size()) f.push_back(payload.substr(start));
  ...
  auto to_float = [](const std::string &s) -> float { return s.empty() ? NAN : atof(s.c_str()); };
  ...
  if (grid_voltage_) grid_voltage_->publish_state(to_float(f[0]) / 10.0f);
  if (grid_frequency_) grid_frequency_->publish_state(to_float(f[1]) / 10.0f);
  if (ac_output_voltage_) ac_output_voltage_->publish_state(to_float(f[2]) / 10.0f);
  if (ac_output_frequency_) ac_output_frequency_->publish_state(to_float(f[3]) / 10.0f);
  if (output_apparent_power_) output_apparent_power_->publish_state(to_float(f[4]));
  if (output_active_power_) output_active_power_->publish_state(to_float(f[5]));
  if (load_percent_) load_percent_->publish_state(to_float(f[6]));
  if (battery_voltage_) battery_voltage_->publish_state(to_float(f[7]) / 10.0f);
  if (battery_discharge_current_) battery_discharge_current_->publish_state(to_float(f[10]));
  if (battery_charge_current_) battery_charge_current_->publish_state(to_float(f[11]));
  if (battery_capacity_) battery_capacity_->publish_state(to_float(f[12]));
  if (heatsink_temperature_) heatsink_temperature_->publish_state(to_float(f[13]));
  if (pv1_power_) pv1_power_->publish_state(to_float(f[16]));
  if (pv1_voltage_) pv1_voltage_->publish_state(to_float(f[18]) / 10.0f);

  return true;
}

void PI18Component::publish_mode_(uint8_t code) {
  if (!mode_text_) return;
  const char *text = "?";
  // PI18 MOD map per protocol doc:
  // 00 Power on, 01 Standby, 02 Bypass, 03 Battery, 04 Fault, 05 Hybrid (Line), etc.
  switch (code) {
    case 0: text = "PowerOn"; break;
    case 1: text = "Standby"; break;
    case 2: text = "Bypass"; break;
    case 3: text = "Battery"; break;
    case 4: text = "Fault"; break;
